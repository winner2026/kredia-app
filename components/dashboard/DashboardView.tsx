...
export default function DashboardView({
  ...
  const [resumen, setResumen] = useState<FormData | null>(
    initialCard ? { ...initialForm } : null,
  );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newPurchase: Purchase = {
        ...
        cuotasPagadas: created
          ? created.installments - created.remaining
          : cuotasPagadas,
      ...
      if (editingId) {
        setPurchases((prev) =>
          prev.map((p) => (p.id === editingId ? newPurchase : p)),
        );
        setEditingId(null);
      } else {
  ...
  const [monthlyTotals, setMonthlyTotals] = useState<
    { label: string; total: number; dueDates: Date[] }[]
  >([]);
  const [freedomDate, setFreedomDate] = useState<Date | null>(null);
  const [scheduleMap, setScheduleMap] = useState<
    Record<string, InstallmentSchedule[]>
  >({});
  ...
  const freedomDateLabel = useMemo(() => {
    if (!hasCalendar || purchases.length === 0 || !freedomDate)
      return "Ya estas a $0";
    return freedomDate.toLocaleDateString("es-ES", {
      day: "2-digit",
      month: "short",
      year: "numeric",
    });
  ...
  const getEndDateLabel = (purchase: Purchase) => {
    ...
    if (!last) return "-";
    return last.dueDate.toLocaleDateString("es-ES", {
      month: "short",
      year: "numeric",
    });
  };
  ...
  const latestSchedule = latestPurchase
    ? (scheduleMap[latestPurchase.id] ?? [])
    : [];
    ...
    <div className="min-h-screen bg-slate-950 text-slate-50">
      <div className="mx-auto max-w-4xl space-y-10 px-6 py-14">
        <header className="space-y-2 text-center">
          <h1 className="text-5xl font-semibold tracking-tight text-[#E13787]">
            Dashboard de compras
          </h1>
          <p className="text-sm text-slate-400">
            Registra las compras de tu tarjeta y visualiza un resumen de forma
            clara y profesional.
          </p>
        </header>

        {purchases.length > 0 && (
          <section className="space-y-4 rounded-lg border border-white/10 bg-white/5 p-6 shadow-lg">
        ...
        {purchases.length > 0 && (
          <section className="space-y-4 rounded-lg border border-white/10 bg-white/5 p-6 shadow-lg">
            <div className="relative flex items-center justify-center gap-3 text-center">
              <h3 className="text-2xl font-semibold text-yellow-300 underline underline-offset-4">
              ...
              <div className="relative">
                ...
                <div className="grid grid-cols-12 items-end gap-2 sm:gap-3">
                  {monthlyTotals.map((m, idx) => {
                    const heightPercent =
                      displayMaxTotal > 0
                        ? Math.min(100, (m.total / displayMaxTotal) * 100)
                        : 0;
                    ...
                    const fallbackDue = hasCalendar
                      ...
                      : undefined;
                    const dueDate =
                      (m.dueDates?.[0] as Date | undefined) ?? fallbackDue;
                    ...
                    const now = new Date();
                    const isOverdue =
                      isCurrentMonth && dueDate ? now > dueDate : false;
                    return (
                      <div
                        key={idx}
                      ...
                      >
                        ...
                        <span className="mt-1 text-[11px] uppercase tracking-wide text-slate-300">
                          {m.label}
                        </span>
                        {dueLabel && (
                          <span
                            className={`text-[10px] ${isOverdue ? "text-rose-300" : "text-cyan-200"}`}
                          >
        ...
        {purchases.length > 0 && (
          <section className="space-y-4 rounded-lg border border-white/10 bg-linear-to-br from-slate-900 via-slate-950 to-slate-900 p-6 shadow-lg">
            <h3 className="text-xl font-semibold text-[#27C1D0]">
              Insights inteligentes
            </h3>
            <div className="grid gap-3 md:grid-cols-3">
              <div className="rounded-xl border border-cyan-500/20 bg-white/5 p-4 text-sm text-slate-200 shadow">
                <p className="text-xs uppercase tracking-[0.14em] text-cyan-200">
                  Optimizacion por cierre
                </p>
                <p className="mt-2 font-semibold">
                  {diasParaProximoPeriodo === null
                    ? "Completa dia de cierre y fecha de compra"
                    : diasParaProximoPeriodo === 0
                      ? "Si compras hoy, ya cae en el proximo periodo."
                      : `Si esperas ${diasParaProximoPeriodo} dias para comprar, esta compra cae en el proximo periodo y te alivia este mes.`}
                </p>
              </div>
              <div className="rounded-xl border border-fuchsia-500/20 bg-white/5 p-4 text-sm text-slate-200 shadow">
                <p className="text-xs uppercase tracking-[0.14em] text-fuchsia-200">
                  Fecha exacta de liberacion
                </p>
                <p className="mt-2 font-semibold">
                  {ultimaCuotaFecha ? (
                    <>
                      Con esta compra quedaras libre el{" "}
                    ...
                    "Agrega una compra y te mostramos la fecha de tu ultima cuota."
                  )}
                </p>
              </div>
              <div className="rounded-xl border border-emerald-500/20 bg-white/5 p-4 text-sm text-slate-200 shadow">
                ...
                <p className="mt-2 font-semibold">
                  {nextDueDate ? (
                    ...
                    "Cuando registres compras con dia de cierre y vencimiento, activamos alertas precisas."
                  )}
                </p>
              </div>
            </div>
        ...
        <section className="rounded-lg border border-white/10 bg-white/5 p-6 shadow-lg">
          <h2 className="mb-4 text-xl font-semibold text-[#F8B738]">
            Registrar compra
          </h2>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleChange("nombreTarjeta", e.target.value)
                  }
              ...
              <div className="space-y-2">
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleChange("diaCierre", clampDayInput(e.target.value))
                  }
                  placeholder="Ej. 20"
                  disabled={cardLocked}
                />
            ...
            <div className="space-y-3">
              {purchases.map((p) => {
                ...
                return (
                  <div
                    key={p.id}
                    className="flex flex-col gap-2 rounded-md border border-white/10 bg-white/5 p-4 text-sm text-slate-100"
                  >
                    <p>
                      <span className="font-medium">Nombre de la compra: </span>
                      <span className="font-semibold text-[#F8B738]">
                        {formatDescription(p.descripcion)}
                      </span>
                    </p>
                    <p>
                    ...
                    {firstPending && (
                      <p className="text-cyan-200">
                        ...
                        {firstPending.dueDate.toLocaleDateString("es-ES", {
                          day: "2-digit",
                          month: "short",
                        })}
                        .
